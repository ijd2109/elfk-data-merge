...
# updated by Sean Minns Dec 2018
...
IF_Q <- read.csv("/Volumes/danl/ELFK/Data_Entry/Questionnaires/R_Cleaning_Qualtrics_Data/QUALTRICS_RAW_FOR_R/Qualtrics_Raw_Indiscriminate_Friendliness_Questionnaire.csv", stringsAsFactors = FALSE)
#view
head(IF_Q)
names(IF_Q)
nrow(IF_Q)
...
# we are entering data directly from RAW FORM INTO R HERE.
...
write.csv(IF_Q.3, "/Volumes/danl/ELFK/Data_Entry/Questionnaires/R_Cleaning_Qualtrics_Data/QUALTRICS_CLEANED_IN_R/Qualtrics_Cleaned_in_R_IF_Questionnaire.csv", row.names = FALSE, quote = F)
