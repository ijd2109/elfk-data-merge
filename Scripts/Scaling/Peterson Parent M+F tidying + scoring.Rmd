...
#check gender 
###if any IDs don't agree with a child's gender on the demographics form, then the ID may have been incorrectly entered, and the data may actually refer to a sibling.
```{r}
...
#conclusion: the entry "EL041P" refers to "EL042"
df.m$Q1_1[which(df.m$Q1_1 == "EL041P")] <- "EL042P" #note, this is the exact match
#Further: EL127 has a parent-entered entry in both male and female forms, but is female
rem1 <- which(df.m$Q1_1 == "EL127")#note, this is the exact match
df.m <- df.m[-rem1,]
```
...
#df.time <- df.time[-which(df.time$id.generated == "NANA"),] #DON'T RUN
...
unk.id.gen <- df.timefull[df.timefull$parent.or.child == "",] %>%
  select(id.generated,id,gender, Timestamp,
         child.timestamp,parent.timestamp1, parent.timestamp2)
search.out <- NULL
...
#In addition, 87 has a sibling [86], and the parent fills out their forms with the ID for that child [86p], so 87 is the older child
...
#RECLASSIFY 1 entry
```{r}
#Based on these results, "el041" is the parent entry to accompany "041".
#Switch it to the correct data frame
DATA.EL041p <- ppds.F_c[grep("el041", ppds.F_c$Q1_1),]
#rename these columns before rbind-ing
names(DATA.EL041p) <- names(ppds.F_p)
ppds.F_p <- rbind(ppds.F_p, DATA.EL041p)
```

#results also suggest to retain the earlier entry of EL005
```{r}
#remove the parent's entry of el041 from the child data frame.
#note the retained entry of EL005 is on the child form.
rem <- c(grep("el041", ppds.F_c$Q1_1), grep("el005", ppds.F_c$Q1_1))
ppds.F_c2 <- ppds.F_c[-rem,]
```
...
###Based on the time the parent filled out entries for siblings, move one entry to parent data frame.
```{r}
DATA.EL004p <- ppds.M_c[grep("el004", ppds.M_c$Q1_1),]
#rename these columns before rbind-ing
names(DATA.EL004p) <- names(ppds.M_p)
ppds.M_p <- rbind(ppds.M_p, DATA.EL004p)
#delete the parent's (retain the child's) etnry from the child data frame
...
```
Conclusion: EL049 is the child, el049 is the parent when taking another look at the timestamps of the scared form, which is parent-report only.
```{r}
#extract and move EL049 to the child data frame
DATA.EL049c <- ppds.M_p[grep("EL049", ppds.M_p$Q1_1),]#verified exact match
names(DATA.EL049c) <- names(ppds.M_c2)
ppds.M_c2 <- rbind(ppds.M_c2, DATA.EL049c)
...
fc3 <- fc2 %>% group_by(IDENT_SUBID) %>%
  ...
  select(-PPDS_F_7_MENSTRUAGE_yr_C,-PPDS_F_7_MENSTRUAGE_mo_C,
               -PPDS_F_8_HEIGHT_ft_C, -PPDS_F_8_HEIGHT_in_C) %>% ungroup()
